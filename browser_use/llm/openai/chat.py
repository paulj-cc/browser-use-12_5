	...
	async def ainvoke(
		...
		with langfuse.start_as_current_observation(
			...
			try:
				...
				generation.update(level="ERROR", status_message="ModelProviderError")
				raise

			except RateLimitError as e:
				generation.update(level="ERROR", status_message=f"RateLimitError: {e.message}")
				raise ModelRateLimitError(message=e.message, model=self.name) from e

			except APIConnectionError as e:
				generation.update(level="ERROR", status_message=f"APIConnectionError: {str(e)}")
				raise ModelProviderError(message=str(e), model=self.name) from e

			except APIStatusError as e:
				generation.update(level="ERROR", status_message=f"APIStatusError {e.status_code}: {e.message}")
